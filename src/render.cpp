...
void Renderer::drawTriangleFilled(Vec3 vertex0, Vec3 vertex1, Vec3 vertex2, Color color, float w, float h) {
    ...
    int row_len = maxx - minx + 1;
    int chunk_count = row_len / chunk_len;
    // Ignore remainder for now!
    int chunk_rem = row_len % chunk_len;
    xVec3 v0 = xVec3::from_vec(vertices[0]);
    xVec3 v1 = xVec3::from_vec(vertices[1]);
    xVec3 v2 = xVec3::from_vec(vertices[2]);
    xVec3 v0v1 = v1 - v0;
    xVec3 v0v2 = v2 - v0;
    xVec3 N = v0v1.cross(v0v2);
    if (std::abs(N.z.first()) < 1e-9) return;
    // float inverse_nz = 1.0 / N.z;
    xsimd::batch<float> inverse_N = xsimd::batch<float>{1.0f, 1.0f, 1.0f, 1.0f} / N.z;
    ...
    for (int y = miny; y <= maxy; y++) {
        ...
        for (int ix = 0; ix < chunk_count; ix++) {
            int x0 = minx + chunk_len * ix;

            float x0f = static_cast<float>(x0);
            xsimd::batch<float> xf = xoffsets + x0f;
            ...
            xsimd::batch<float> dot0 = v1v2.cross(v1p) * N;
